      ...
      <div className="absolute inset-0 z-50 pointer-events-none flex flex-col">
        ...
        <header className="h-16 border-b border-white/5 bg-slate-900/40 backdrop-blur-md flex items-center justify-between px-8 pointer-events-auto">
          <div className="flex items-center gap-4 cursor-pointer" onClick={() => setViewMode('DASHBOARD')}>
            ...
            <span className="font-black text-2xl text-white tracking-tighter uppercase italic">DUM-E 3D</span>
          </div>
            ...
            {['MANUAL', 'SEQUENCE', 'LEARN', 'FIRMWARE'].map((mode) => (
              <button
                ...
                className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${
                  viewMode === mode ? 'bg-cyan-500/20 text-cyan-400 border border-cyan-500/50 shadow-lg' : 'text-slate-400 hover:text-white'
                }`}
              >
                {mode}
              </button>
            ))}
            <button onClick={serial.connect} className="bg-cyan-400 text-black px-6 py-2 rounded-full font-bold ml-4 active:scale-95 transition-transform">
        ...
        <div className="flex-1 relative">
          <aside className="absolute left-10 top-10 bottom-10 w-[420px] flex flex-col gap-6">
            <div className="glass-panel rounded-[2.5rem] p-10 pointer-events-auto overflow-y-auto max-h-full scrollbar-hide text-white">
              ...
              <div className="mt-8 pt-8 border-t border-white/10">
                <button 
                  ...
                  className={`w-full flex items-center justify-center gap-3 p-5 rounded-3xl font-bold transition-all ${isExploded ? 'bg-cyan-400 text-black shadow-xl' : 'bg-slate-800 text-slate-400 hover:text-white'}`}
                >
